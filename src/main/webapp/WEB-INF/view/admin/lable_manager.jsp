...
<div class="tpl-page-container tpl-page-header-fixed">
        ...
        <div class="tpl-left-nav-list">
            <ul class="tpl-left-nav-menu">
                ...
                <li class="tpl-left-nav-item">
                    ...
                    <ul class="tpl-left-nav-sub-menu" style="display: block;">
                        <li>
                            <a href="${ctx}/lable_manager" class="nav-link active">
                                <i class="am-icon-angle-right"></i>
                                <span>标签云</span>
                                <i class="am-icon-star tpl-left-nav-content-ico am-fr am-margin-right"></i>
                            </a>
    ...
    <div class="tpl-content-wrapper">
        ...
        <ol class="am-breadcrumb">
            ...
            <li class="am-active">标签云列表</li>
        </ol>
        <div class="tpl-portlet-components">
            <div class="portlet-title">
                <div class="caption font-green bold">
                    <span class="am-icon-code"></span> 标签云
                </div>
                    ...
                    <div class="am-u-sm-12">
                        <table class="am-table am-table-striped am-table-hover table-main">
                            ...
                            <tr>
                                ...
                                <th class="table-title">标签云名称</th>
                                <th class="table-author am-hide-sm-only">标签云地址</th>
                                <th class="table-set">操作</th>
                            </tr>
                            </thead>
                            <tbody>
                            <c:forEach items="${pageInfo.list}" var="lable">
                                <tr>
                                    <td>${lable.id}</td>
                                    <td><a href="#">${lable.lableName}</a></td>
                                    <td><a href="#">${lable.lableUrl}</a></td>
                                    <td>
                                        <div class="am-btn-toolbar">
                                            <div class="am-btn-group am-btn-group-xs">
                                                <button onclick="return updateLable(${lable.id})" class="am-btn am-btn-default am-btn-xs am-text-secondary">
                                                    <span class="am-icon-edit"></span>编辑
                                                </button>
                                                <button onclick="return delLable(${lable.id})" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only">
                            ...
                            <div class="am-fr">
                                <ul class="am-pagination tpl-pagination">
                                    <c:if test="${pageInfo.hasPreviousPage}">
                                        <li><a href="${ctx}/lable_manager?pn=${pageInfo.pageNum-1}">«</a></li>
                                    </c:if>

                                    <c:if test="${not pageInfo.hasPreviousPage}">
                                        <li style="cursor:not-allowed;" class="am-disabled"><a
                                                href="${ctx}/lable_manager?pn=${pageInfo.pageNum-1}">«</a></li>
                                    </c:if>
                                    ...
                                    <c:forEach items="${pageInfo.navigatepageNums}" var="page_Num">
                                        ...
                                        <c:if test="${page_Num != pageInfo.pageNum}">
                                            <li><a href="${ctx}/lable_manager?pn=${page_Num}">${page_Num}</a></li>
                                        </c:if>
                                    </c:forEach>
...
<script>
    ...
    $("#add").click(function () {
        layer.open({
            ...
            ,content: '<div style="padding: 30px; line-height: 22px; background-color: #fff; color: #fff; font-weight: 300;">' +
                ...
                '                      action="${ctx}/doAddLable" method="post"\n' +
                '                >\n' +
                '\n' +
                '\n' +
                '                    <div class="am-form-group">\n' +
                '                        <label for="title" class="am-u-sm-3 am-form-label">名称<span class="tpl-form-line-small-title">LableName</span></label>\n' +
                '                        <div class="am-u-sm-9">\n' +
                '                            <input type="text" class="tpl-form-input" id="title" name="lableName"  \n' +
                '                                   placeholder="请输入名称">\n' +
                ...
                '\n' +
                '                    <div class="am-form-group">\n' +
                '                        <label for="user-intro" class="am-u-sm-3 am-form-label">链接<span class="tpl-form-line-small-title">LableURL</span></label>\n' +
                '                        <div class="am-u-sm-9">\n' +
                '                            <input type="text" class="tpl-form-input" id="title" name="lableUrl"  \n' +
                '                                   placeholder="请输入地址">\n' +
     ...
     * @param id
     */
    function updateLable(id) {
        $.ajax({
            type:"POST",
            url:"${ctx}/editLablePage",
            data:{
                "id":id
            },
            success:function (result) {
                if(result.msg=='成功'){
                    var id = result.info.lables.id;
                    var linkname = result.info.lables.lableName;
                    var linkurl = result.info.lables.lableUrl;
                    layer.open({
                        type: 1
                        ,title: false //不显示标题栏
                        ,closeBtn: false
                        ...
                        ,content: '<div style="padding: 30px; line-height: 22px; background-color: #fff; color: #fff; font-weight: 300;">' +
                            '<form class="am-form tpl-form-line-form"\n' +
                            '                      action="${ctx}/doEditLable" method="post"\n' +
                            '                >\n' +
                            '\n' +
                            '\n' +
                            ...
                            '                        <div class="am-u-sm-9">\n' +
                            '                            <input type="text" class="tpl-form-input" id="title" name="lableName" value="'+linkname+'"\n' +
                            '                                   placeholder="请输入标签云名称">\n' +
                            '                            <small id="categoryname"></small>\n' +
                            '                        </div>\n' +
                            '                    </div>\n' +
                            '\n' +
                            ...
                            '                        <div class="am-u-sm-9">\n' +
                            '                            <input type="text" class="tpl-form-input" id="title" name="lableUrl"  \n' +
                            '                                value="'+linkurl+'"   placeholder="请输入标签云地址">\n' +
                            '                            <small id="deprecated"></small>\n' +
                            '                        </div>\n' +
                            '                    </div>\n' +
                            '\n' +
